...
ContextCite is the headline method and the one whose behaviour the project set out to probe. It samples random ablations of the context — each chunk included with probability one half — teacher-force-scores the *fixed* original answer under each ablation to get the answer's log-probability, transforms it to the logit of the response probability — the target the method regresses, computed with a numerically stable `log1mexp` and clamped so a fully confident answer stays finite — and fits a sparse linear surrogate (Lasso) of that target against the binary inclusion mask. The learned weight on each chunk is its attribution; the predicted culprit is the largest weight.
