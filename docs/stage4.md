...
**Counterfactual effect (the causal axis).** The chunk is removed from the context, the model is re-run on what remains, and the new answer is compared to the original *by value*: each answer is reduced to a canonical key — the gold value, the planted-wrong value, or its normalized text — so that a rephrasing carrying the same value does not count as a change. If that value changes, the chunk was causal — it was load-bearing for the answer the model actually gave. This is a threshold-free, directly interpretable criterion that matches the build plan's "answer shift" while ignoring cosmetic rewording. The stage also records the stronger event of the answer becoming *correct* once the chunk is removed (the wrong→right flip that pinpoints an error's cause) and, as a continuous measure, the drop in the original answer's log-probability when the chunk is removed.

**Apparent salience (the looks-responsible axis).** A chunk is salient if it holds the model's answer — either the answer string outright, or, when the model answered verbosely, the canonical value (the gold or the planted-wrong) that the answer carries. This is the surface cue a salience-based method, or a person skimming, would latch onto, and it is again transparent and threshold-free. Keeping arbitrary cutoffs out of the ground truth is deliberate: a label that depended on a tuned threshold would be a label a reviewer could argue with.
