...
def parse_example(raw: dict, source: str = "hotpotqa") -> QAExample:
    """Turn one multi-hop QA row into a QAExample. HotpotQA and 2WikiMultiHopQA share this
    layout — context paragraphs keyed by title, with annotated supporting sentences — so the
    only thing that differs is the recorded `source`."""
    ...
    return QAExample(
        ...
        meta={"type": raw.get("type"), "level": raw.get("level"), "source": source},
    )
